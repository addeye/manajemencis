        ...
        <div class="col-xs-12">
            ...
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Edit Data Pelaksanaan Pendampingan</h3>
                </div>
                <!-- / box Header -->
                <div class="box-body">
                    <form method="post" action="{{ url('pelaksanaan-pendampingan/'.$data->id) }}" class="form-horizontal">
                        ...
                        <div class="form-group {{$errors->has('program_kerja_id')?'has-error':''}}">
                            ...
                            <select name="program_kerja_id" class="form-control select2">
                                ...
                                @forelse ($program_kerja as $row)
                                    <option value="{{$row->id}}" {{$data->program_kerja_id==$row->id?'selected':''}}>{{$row->proker_pendampingan}} - {{$row->sasaran_program->nama_kumkm}}</option>
                                @empty
                        ...
                        <div class="form-group {{$errors->has('tanggal')?'has-error':''}}">
                            ...
                            <div class="col-sm-5">
                                <input type="text" name="tanggal" class="form-control" placeholder="Tanggal Pelaksanaan Pendampingan" value="{{$data->tanggal?date('d-m-Y',strtotime($data->tanggal)):date('d-m-Y')}}" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask>
                        ...
                        <div class="form-group {{$errors->has('materi')?'has-error':''}}">
                            ...
                            <div class="col-sm-5">
                                <input type="text" class="form-control" name="materi" placeholder="Materi Pendampingan" value="{{$data->materi}}">
                        ...
                        <div class="form-group {{$errors->has('tindak_lanjut')?'has-error':''}}">
                            ...
                            <div class="col-sm-5">
                                <textarea name="tindak_lanjut" placeholder="Tindakan Lebih Lanjut" class="form-control">{{$data->tindak_lanjut}}</textarea>
