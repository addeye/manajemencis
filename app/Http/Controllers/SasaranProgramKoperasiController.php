...
class SasaranProgramKoperasiController extends Controller
{
    ...
    public function lock(Request $request, $id)
    {
        ...
        if ($data) {
            return redirect('sasaran-program-koperasi')->with('success', 'Status LOCK menjadi ' . $lock);
        }
    }
    ...
    {
        $sasaran_koperasi_id = $request->sasaran_koperasi_id_to_lock;
        $jumlah = 0;
        $gagal = 0;

        if (!$sasaran_koperasi_id) {
            return redirect('sasaran-program-koperasi')->with('error', 'Pastikan Memilih Sasaran nya. Harus Teliti lagi ! ');
        }
        foreach ($sasaran_koperasi_id as $key => $value) {
    ...
    {
        $sasaran_koperasi_id = $request->sasaran_koperasi_id_to_unlock;
        $jumlah = 0;
        $gagal = 0;
