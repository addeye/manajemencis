...
class CommonController extends Controller {
	...
	public function getKonsultans() {
		$data = array(
			'title' => 'Informasi Pasar',
			'konsultans' => Konsultan::whereHas('user', function ($q) {
				$q->where('status', 'aktif');
			})->get(),
		);
		return view('common.informasi_konsultan', $data);
	}

	public function getPlut() {
		$data = array(
			'title' => 'Informasi Pasar',
			'lembaga' => Cis_lembaga::with(['konsultan' => function ($q) {
				$q->whereHas('user', function ($k) {
					$k->where('status', 'aktif');
				});
			}])->get(),
		);
		// return $data;
		return view('common.informasi_cis', $data);
	}
}
