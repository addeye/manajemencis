			...
			<div class="box">
				...
				<div class="box-body table-responsive">
					...
					<div class="col-md-2 row">
						<form id="formchecklock" method="post" action="{{ url('sasaran-koperasi-pendampingan-lock') }}" onsubmit="return confirm('Apakah anda yakin ? ')">
							{{ csrf_field() }}
							<div class="form-group">
					...
					<div class="col-md-2 row">
						<form id="formcheckunlock" method="post" action="{{ url('sasaran-koperasi-pendampingan-unlock') }}" onsubmit="return confirm('Apakah anda yakin ? ')">
							{{ csrf_field() }}
							<div class="form-group">
							...
							<tbody>
								...
								@foreach ($data as $row)
									<tr>
										...
										<td align="center"><input id="checksub" type="checkbox" name="sasarankoperasi_id[]" class="checkSasaranKoperasi" value="{{$row->id}}"></td>
										<td>{{$no++}}</td>
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		$("#checkAll").change(function() {
            if(this.checked) {
                $('.checkSasaranKoperasi').prop('checked',true);
                $('.checkSasaranKoperasi').each(function(){
                	$('.inputselectlock').append('<input id="saskoplock'+this.value+'" type="hidden" name="sasaran_koperasi_id_to_lock[]" value="'+this.value+'"/>');
                	$('.inputselectunlock').append('<input id="saskopunlock'+this.value+'" type="hidden" name="sasaran_koperasi_id_to_unlock[]" value="'+this.value+'"/>');
                });
            }
            else
            {
              $('.checkSasaranKoperasi').prop('checked',false);
              $('.inputselectlock').html('');
              $('.inputselectunlock').html('');
            }
        });

        $(".checkSasaranKoperasi").change(function() {
            if(this.checked) {
                $('.inputselectlock').append('<input id="saskoplock'+this.value+'" type="hidden" name="sasaran_koperasi_id_to_lock[]" value="'+this.value+'"/>');
                $('.inputselectunlock').append('<input id="saskopunlock'+this.value+'" type="hidden" name="sasaran_koperasi_id_to_unlock[]" value="'+this.value+'"/>');
            }
            ...
            {
              $('#saskoplock'+this.value).remove();
              $('#saskopunlock'+this.value).remove();
            }
        });
    });
</script>
