...
<head>
    ...
    <title>KARTU PELAKSANAAN PENDAMPINGAN KOPERASI DAN UMKM {{$tahun}}<br>{{$title}}<br>{{$title}}</title>
    <style>
        table{
            font-size: 12px;
        }
...
<body>
    <div align="center">
        <h4>KARTU PELAKSANAAN PENDAMPINGAN KOPERASI DAN UMKM {{$tahun}}<br>{{$title}}</h4>
        <table align="left">
            <tr>
                <td>NAMA KONSULTAN PENDAMPING : {{$konsultan->nama_lengkap}}</td>
            </tr>
            <tr>
                <td>NAMA KOPERASI DAN UMKM : {{$sasaran->ukmtable->nama_kumkm}}</td>
            </tr>
        </table>
