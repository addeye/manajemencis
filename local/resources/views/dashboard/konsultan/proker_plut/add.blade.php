	...
	<div class="row">
		<div class="col-xs-12">
			...
			<div class="box">
				...
				<!-- / box Header -->
				<div class="box-body">
					<form action="{{ url('adm/proker-plut') }}" method="post" class="form-horizontal">
						{{ csrf_field() }}
						<div class="form-group {{$errors->has('tahun')?'has-error':''}}">
                            <label class="col-sm-2 control-label">Tahun</label>
                            <div class="col-sm-2">
                                <input type="text" name="tahun" class="form-control" readonly="true" value="{{date('Y')}}">
                        ...
                        <div class="form-group {{$errors->has('indikator')?'has-error':''}}">
                            ...
                            <div class="col-sm-5">
                                <input type="text" class="form-control" name="indikator" placeholder="indikator kinerja.." value="{{old('indikator')}}">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control select2" name="anggaran[]" multiple="multiple">
                                	...
                                	@foreach ($anggaran as $row)
                                		<option value="{{$row->id}}" @if (old("anggaran")){{ (in_array($row->id, old("anggaran")) ? "selected":"") }}@endif >{{$row->nama}}</option>
                                	@endforeach
                                </select>
