			...
			<div class="box">
				...
				<div class="box-body table-responsive">
					...
					<div class="">
                       	...
                       	<form id="formcheck" method="post" action="{{ url('database-koperasi-delete') }}" onsubmit="return confirm('Apakah anda yakin ? ')">
                       		{{ csrf_field() }}
                       		<div class="form-group">
								...
								@foreach ($data as $row)
									<tr>
										<td>
											...
											<form id="delete-form-{{$row->id}}" action="{{ url('database-koperasi/'.$row->id) }}" method="POST" style="display: none;">
												{{ csrf_field() }}
						                        <input type="hidden" name="_method" value="DELETE">
						                    </form>
										</td>
										<td align="center"><input id="checksub" type="checkbox" name="koperasi_id[]" class="checkUser" value="{{$row->id}}"></td>
										<td>{{$no++}}</td>
