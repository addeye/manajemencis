...
class ProgramKerjaController extends Controller
{
    ...
    {
        $program_id = $request->program_id_to_lock;
        $jumlah = 0;
        $gagal = 0;

        if (!$program_id) {
            return redirect('program-kerja-pendampingan')->with('error', 'Pastikan Memilih Program nya. Harus Teliti lagi ! ');
        }
        foreach ($program_id as $key => $value) {
            $prog = ProgramKerja::find($value);
            if ($prog) {
                $prog->lock = 'Yes';
                $prog->save();
                if ($prog) {
        ...
        return redirect('program-kerja-pendampingan')->with('info', $jumlah . ' Data Telah Lock, Gagal Sebanyak ' . $gagal);
    }

    public function multipleUnlock(Request $request)
    {
        $program_id = $request->program_id_to_unlock;
        $jumlah = 0;
        $gagal = 0;
        ...
        return redirect('program-kerja-pendampingan')->with('info', $jumlah . ' Data Telah Unlock, Gagal Sebanyak ' . $gagal);
    }
