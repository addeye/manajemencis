...
class LaporanController extends Controller {
	...
	private function getProgresData($tahun) {
		$result = Cis_lembaga::withCount([
			...
			'kegiatan_konsultan AS total_kegiatan' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			},
			'kegiatan_konsultan AS kegiatan_by_kelembagaan' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 1)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			}, 'kegiatan_konsultan AS kegiatan_by_sdm' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 2)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			}, 'kegiatan_konsultan AS kegiatan_by_produksi' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 3)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			}, 'kegiatan_konsultan AS kegiatan_by_pembiayaan' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 4)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			}, 'kegiatan_konsultan AS kegiatan_by_pemasaran' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 5)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			}, 'kegiatan_konsultan AS kegiatan_by_it' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 6)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
			}, 'kegiatan_konsultan AS kegiatan_by_kerjasama' => function ($q) use ($tahun) {
				$q->whereHas('konsultan', function ($q) use ($tahun) {
					$q->where('bidang_layanan_id', 7)->whereHas('kegiatan', function ($q) use ($tahun) {
						$q->whereYear('created_at', $tahun);
					});
				});
