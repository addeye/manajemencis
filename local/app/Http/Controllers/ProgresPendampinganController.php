...
class ProgresPendampinganController extends Controller
{
    ...
    {
        $start = Input::has('start') ? date('Y-m-d', strtotime(Input::get('start'))) : '';
        $end = Input::has('end') ? date('Y-m-d', strtotime(Input::get('end'))) : '';
        ...
        return view('progres_pendampingan.list', $data);
    }
